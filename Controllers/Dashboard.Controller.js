...
function atualizarDashboard() {
  ...
  planilhaDashboard.getRange("A1").setValue("Dashboard de Gerenciamento de Dispositvos");
  ...
  const tiposDispositivos = ["Paleteira Manual", "Paleteira Elétrica", "Empilhadeira Elétrica", "Empilhadeira a Gás", "Empilhadeira Diesel", "Celular", "Notebook", "Tablet", "Impressora", "Monitor", "Projetor", "Teclado", "Mouse", "Webcam"];
  ...
  [...dadosPatio.slice(1), ...dadosDispositivos.slice(1)].forEach(linha => {
    if (linha[1].toLowerCase().includes(tipo.toLowerCase())) {
      ...
      if (linha[2] === "Disponível") {
        if (tiposDispositivos.some(t => tipo === t)) {
          noPatio++;
        } else {
          noDispositivos++;
        }
  ...
  // Verificação condicional para Paleteiras e Empilhadeiras
  if (["Paleteira Manual", "Paleteira Elétrica", "Empilhadeira Elétrica", "Empilhadeira a Gás", "Empilhadeira Diesel"].some(t => tipo === t)) {
